...
def add_arguments(parser):
  ...
  # Metadata info
  parser.add_argument("--run_metadata", type="bool", nargs="?", const=True,
                    default=False,
                    help="Whether to collect metadata.")   
  
  parser.add_argument("--do_profile", type="bool", nargs="?", const=True,
                    default=False,
...
def create_hparams(flags):
  ...
  return tf.contrib.training.HParams(
      ...
      run_metadata=flags.run_metadata,
      do_profile=flags.do_profile,
  )
...
def run_main(flags, default_hparams, train_fn, inference_fn, target_session=""):
  ...
  if flags.hparams_path:
    for param_name, param_value in hparams.values().items():
        for flag_name, flag_value in user_flags.items():
            if param_name == flag_name and param_value != flag_value:
                print("overwriting {0} from {1} to {2}".format(
                    param_name, param_value, flag_value))
                param_value = flag_value
...
if __name__ == "__main__":
  ...
  for f in sys.argv[1:]:
    m = re.match("--(.*)=(.*)", f)
    if m:
        user_flags[m.group(1)] = m.group(2)
